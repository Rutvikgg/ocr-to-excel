...
def open():
    ...
    #                                                                                                              Descryption  type
    root.filename = filedialog.askopenfilename( title="Select a File", filetypes=(("jpg files",".jpg"),("png files",".png"),("all files",".*")))

    path_label=Label(frame,text=root.filename,borderwidth=2,relief="solid").grid(row=0,column=0,columnspan=2,padx=5,pady=5)
    my_image=ImageTk.PhotoImage(Image.open(root.filename))
    my_image_label=Label(inner_frame, image=my_image).grid(row=0,column=0)

primary_heading=Label(root, text="OCR Your Document",pady=5, anchor=E).grid(row=0,column=0,sticky="w")
...
inner_frame=LabelFrame(root,bg="#88d4ab", padx=10,pady=10,borderwidth=5)
inner_frame.grid(row=1,column=0,padx=25,pady=25, columnspan=3)

# my_button=Button(frame,text="Open file").grid(row=0,column=0, pady=10)

separator3 = ttk.Separator(root, orient="horizontal")
separator3.grid(row=6,column=0,sticky="ew",columnspan=2)

specify_label=Label(root, text="Specify Document Type").grid(row=7,column=0,sticky="w")

description_label=Label(root, text="Specifying the document leads to better result and extraction of data").grid(row=8,column=0)
...
back_btn=Button(root,text="Back").grid(row=10,column=0, columnspan=1,sticky="w",padx=10)
back_btn1=Button(root,text="Start Conversion").grid(row=10,column=2, columnspan=1,sticky="e")
